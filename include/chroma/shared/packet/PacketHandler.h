...
class PacketHandler {
public:
    static std::vector<uint8_t> GameObjectsToFlatBuffer(const std::unordered_map<UUIDv4::UUID, std::shared_ptr<chroma::shared::core::GameObject>>& objects, const UUIDv4::UUID& player_id, uint64_t time_lapse = 0, uint32_t last_processed_input = 0);

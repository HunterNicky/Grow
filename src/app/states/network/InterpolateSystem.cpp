...
void InterpolateSystem::Interpolate(std::unordered_map<UUIDv4::UUID, std::shared_ptr<chroma::shared::core::GameObject>>& new_snapshot, uint64_t delta_time)
{
...
void InterpolateSystem::InterpolatePosition(
    const std::shared_ptr<chroma::shared::core::GameObject>& past_object,
    const std::shared_ptr<chroma::shared::core::GameObject>& target_object,
    std::shared_ptr<chroma::shared::core::GameObject>& out_object,
    float alpha)
{
    if(out_object->GetId() == player_id_)
    {
        return;
    }
...
uint64_t InterpolateSystem::GetTimeLastSnapshot() const { 
    return time_last_snapshot_; 
}

void InterpolateSystem::SetSnapshotInterval(uint64_t interval) { 
    snapshot_interval_ = interval; 
}

void InterpolateSystem::Update(uint64_t delta_time)
{
    if (!game_objects_ || past_game_objects_.empty() || target_game_objects_.empty())
    {
        return;
    }
...
void InterpolateSystem::SetGameObjects(const std::shared_ptr<std::unordered_map<UUIDv4::UUID, std::shared_ptr<chroma::shared::core::GameObject>>>& objects)
{
    game_objects_ = objects;
}

void InterpolateSystem::SetPlayerId(const UUIDv4::UUID& player_id)
{

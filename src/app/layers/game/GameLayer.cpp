...
{
  state_factory_.Register(states::StateID::PauseState, []() { return std::make_shared<states::menu::PauseState>(); });

  state_factory_.Register(states::StateID::OptionsMenuState,
    []() { return std::make_shared<states::menu::OptionsMenuState>(states::StateID::PauseState); });
}
...
void GameLayer::OnStateEvent(shared::event::state::StateEvent &state_event)
{
  ...
  case shared::event::state::Action::Pop: {
    ...
    if (state_event.GetStateId() == states::StateID::PauseState && command_queue_->IsEmpty()) {
      if (const auto game_state = std::dynamic_pointer_cast<states::GameState>(state)) { game_state->SetPaused(false); }
    }
  ...
  case shared::event::state::Action::Push: {
    ...
    if (state_event.GetStateId() == states::StateID::PauseState
        || state_event.GetStateId() == states::StateID::OptionsMenuState) {
      auto game_state = std::dynamic_pointer_cast<states::GameState>(this->GetCurrentState());
      if (game_state) { game_state->SetPaused(true); }
    }

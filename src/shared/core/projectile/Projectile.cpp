...
Projectile::Projectile() {
    Type_ = GameObjectType::PROJECTILE;
}
...
std::shared_ptr<GameObject> Projectile::Clone() {
    return std::make_shared<Projectile>(*this);
}

void Projectile::OnFixedUpdate(float delta_time) {
    (void)delta_time;
}

void Projectile::OnCollision(const collision::CollisionEvent& colission_event) {
    (void)colission_event;
}

void Projectile::HandleEvent(const event::Event& event) {
    (void)event;
}

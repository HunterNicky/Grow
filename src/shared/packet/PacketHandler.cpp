    ...
    std::shared_ptr<core::GameObject> &game_object)
  {
    ...
    case Game::GameObjectType::Projectile: {
      ...
      if (projectile) {
        ...
        if(sprite_anim->GetAnimations().empty())
        {
    ...
    std::unordered_map<UUIDv4::UUID, std::shared_ptr<core::GameObject>> &game_objects)
  {
    ...
    for (const auto &entity_state : *snapshot->entities()) {
      ...
      if (const auto it = game_objects.find(entity_id); it != game_objects.end()) {
        ...
        if (game_object) {
          game_objects.emplace(entity_id, game_object);
        }
      }
      ...
      const std::shared_ptr<ProjectileMessage> &projectile_message)
  {
      ...
      const auto fb_dir = Game::CreateVec2(
          builder,
          evt->GetDirection().x,
          evt->GetDirection().y
      );

      const auto fb_position = Game::CreateVec2(
          builder,
          evt->GetPosition().x,
          evt->GetPosition().y
      );

      const auto fb_proj_event =
          Game::CreateProjectileEvent(
              builder,
              fb_id,
              static_cast<Game::TypeProjectile>(evt->GetProjectileType()),
              fb_position,
              fb_dir,
              evt->GetSpeed()
          );

      const auto fb_proj_msg =
          Game::CreateProjectileEventMessage(
              builder,
              projectile_message->GetSeq(),
              projectile_message->GetDeltaTime(),
              fb_proj_event
          );
      ...
      const auto fb_event =
          Game::CreateEvent(
              builder,
              Game::EventUnion::ProjectileEventMessage,
              fb_proj_msg.Union()
          );
  ...
  std::shared_ptr<ProjectileMessage> PacketHandler::EventToProjectileMessage(const Game::Event *evt)
  {
      ...
      if (evt->event_type() != Game::EventUnion::ProjectileEventMessage) {
          return nullptr;
      }
      ...
      if (fb_proj_event != nullptr) {
          ...
          const Vector2 direction{
              fb_proj_event->direction()->x(),
              fb_proj_event->direction()->y()
          };

          const Vector2 position{
              fb_proj_event->position()->x(),
              fb_proj_event->position()->y()
          };

          auto projectile_event = std::make_shared<event::ProjectileEvent>(
              static_cast<core::component::TypeProjectile>(fb_proj_event->type()),
              direction,
              fb_proj_event->speed()
          );

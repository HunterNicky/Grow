  ...
  const uint32_t last_processed_input)
{
  ...
  const auto fb_entities_vec = builder.CreateVector(fb_entities);
  const auto snapshot = Game::CreateSnapshot(builder, time_lapse, last_processed_input, fb_player_id, fb_entities_vec);
  const auto envelope = Game::CreateEnvelope(builder, Game::MsgType::SNAPSHOT, Game::MsgUnion::Snapshot, snapshot.Union());

...
void WaveManager::SpawnEnemies(const uint32_t count) const
{
  ...
  if (grid.empty() || width <= 0 || height <= 0 || tile_size <= 0) { return; }

  auto find_walkable_near = [&](const std::shared_ptr<shared::core::GameObject> &player_obj) {
    if (!player_obj) { return Vector2{ 0.0F, 0.0F }; }
  ...
  for (uint32_t i = 0; i < count; ++i) {
    ...
    const Vector2 spawn_pos = find_walkable_near(player_obj);
    auto enemy = shared::builder::GameObjectBuilder<shared::core::enemy::Enemy>()

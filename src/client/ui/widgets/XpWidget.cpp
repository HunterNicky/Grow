...
XpWidget::XpWidget(const std::string &id, const Rectangle bounds)
  : Widget(id, bounds), level_text_widget_(nullptr), progress_text_widget_(nullptr)
{
  Initialize();
}

void XpWidget::Initialize()
{
  constexpr float border_size = 3.0F;
  constexpr int font_size = 24;
  const float inner_x = bounds_.x + border_size;
  const float inner_y = bounds_.y + border_size;
  const float inner_width = bounds_.width - (2 * border_size);
  const float inner_height = bounds_.height - (2 * border_size);

  const Rectangle level_text_bounds = { inner_x, inner_y, inner_width / 2.0F, inner_height };

  level_text_widget_ = std::make_unique<TextWidget>(id_ + "_level", level_text_bounds, "Nível 1", font_size, WHITE);

  const Rectangle progress_text_bounds = { inner_x + (inner_width / 2.0F), inner_y, inner_width / 2.0F, inner_height };
  progress_text_widget_ =
    std::make_unique<TextWidget>(id_ + "_progress", progress_text_bounds, "0 / 100", font_size, WHITE);
}
...
void XpWidget::OnRender()
{
  ...
  DrawRectangle(static_cast<int>(bounds.x + border_size),
  ...
  const std::string xp_progress_text =
    std::to_string(static_cast<int>(current_xp)) + " / " + std::to_string(static_cast<int>(xp_to_next_level));

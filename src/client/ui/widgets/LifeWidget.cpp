...
LifeWidget::LifeWidget(const std::string &id, const Rectangle bounds) : Widget(id, bounds) {}
...
void LifeWidget::OnRender()
{
  ...
  const std::string health_text =
    std::to_string(static_cast<int>(current_health)) + " / " + std::to_string(static_cast<int>(max_health));

  // Centraliza o texto verticalmente dentro da barra
  const int font_size = 20;
  const float text_x =
    bounds.x + (bounds.width / 2.0F) - (static_cast<float>(MeasureText(health_text.c_str(), font_size)) / 2.0F);
  const float text_y = bounds.y + (bounds.height / 2.0F) - (font_size / 2.0F);

  DrawText(health_text.c_str(), static_cast<int>(text_x), static_cast<int>(text_y), font_size, text_color);
}
